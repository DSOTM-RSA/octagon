...
```{r}
url <-c("Max-Holloway-38671","Neil-Magny-69166","Ricky-Simon-111209","Alexander-Volkanovski-101527","Umar-Nurmagomedov-240893")
...
```{r}
map_df(out$fighterName,fighter_crawler) %>% unique %>% rowid_to_column() -> div_135_145_170 
names(div_135_145_170)[2]<-"fighterName"
```
...
```{r}
map_df(div_135_145_170$fighterName, url_crawler,.id = "rowid") -> results_135_145_170
```
...
# ads uniqueness to results which finish in the same way on the same card i.e. decisions
raw_event_results %>% mutate(IdKey = FighterId.x+FighterId.y)
```


```{r}
raw_event_results[!duplicated(raw_event_results[c("Event.x","Round.x","Time.x","Method.x")]),] -> unique_event_results
```
